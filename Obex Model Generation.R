...
outputFile <- paste(fileFolder, "//RT-QuIC Obex Output Data.xlsx", sep="")
...
##############################################################################################################
#this is a plot of obex ct vs concentration. note that there will be a warning of missing points, this is because this figure only displays 10^-2 to 10^-8
#to include more dilutions, you can change the "scale_y_log10()" parameter

obex <- ggplot(myData, aes(x = Ct, y = Concentration, colour = IHC)) +
...
ggsave("Obex Ct Distribution.png", dpi=1000, dev='png', height = 6, width = 6, units = "in")
...
count <- 1
for(currentConcentration in c(10^-2, 10^-3,10^-4,10^-5,10^-6,10^-7,10^-8, 10^-9,10^-10, 10^-11)){
...
#for example, 10^-3 has p<0.05, but it has a very high false positive rate, so we decided to also exclude that (as recommended by reviewer)
  
#this will only get data from 10^-4 to 10^-8
includedSamples <- myData
includedSamples <- includedSamples[(includedSamples$Concentration<0.5*10^-3),]
...
ggsave("Obex Prediction Graph.png", dpi=1000, dev='png', height = 8, width = 8, units = "in")
...
optimizedCutoffs <- thresholdAnalysis[thresholdAnalysis$Youden==max(thresholdAnalysis$Youden),]
...
ggsave("Obex AUC over time, TMPR over time.png", dpi=1000, dev='png', height = 16, width = 8, units = "in")
...
rateGraph <- ggplot(analysedMPRData, aes(x = Cycle, y = Rate)) +
  ...
  theme_bw(base_size=25)+
  theme(axis.text.y=element_text(size = 20, hjust = 0.5), axis.text.x=element_text(size = 20), panel.grid.major = element_blank(),panel.grid.minor = element_blank())
ggsave("Obex dAUCdT.png", dpi=1000, dev='png', height = 8, width = 8, units = "in")
